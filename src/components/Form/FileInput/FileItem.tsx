...
const fileItem = tv({
  slots: {
    ...
    icon: 'rounded-full border-4 border-orange-100 bg-orange-200 p-2 text-orange-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500',
    deleteButton: '',
  },
  ...
  variants: {
    state: {
      ...
      complete: {
        container: 'border-orange-500 dark:border-orange-300/30',
      },
      error: {
        container:
...
export function FileItem({ name, size, state }: FileItemProps) {
  ...
  return (
    <div className={container()}>
      ...
      {state === 'complete' ? (
        <CheckCircle2 className="h-5 w-5 fill-orange-600 text-white" />
      ) : (
